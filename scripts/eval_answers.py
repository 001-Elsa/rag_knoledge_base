...
JUDGE_PROMPT = """你是严格的问答质量评审。给定「问题 / 参考资料 / 回答」，从两个维度打 1-5 分：
- faithfulness（忠实度）：回答内容是否都能在参考资料中找到依据？编造扣分，明确说"没找到"不扣分。
- relevance（相关性）：回答是否直接切中问题？
只输出 JSON：{"faithfulness": 1-5, "relevance": 1-5, "comment": "一句话点评"}"""
...
def main() -> int:
    ...
    with httpx.Client(base_url=args.base_url, timeout=180) as client:
        ...
        for i, case in enumerate(cases, 1):
            ...
            try:
                ...
                f, r = int(verdict["faithfulness"]), int(verdict["relevance"])
                faith_scores.append(f)
                rel_scores.append(r)
                flag = " ⚠️" if min(f, r) <= 3 else ""
                print(f"[{i}/{len(cases)}] 忠实 {f} / 相关 {r}{flag}  {question}")
                if flag:
    ...
    if faith_scores:
        print(f"\n=== 共 {len(faith_scores)} 条 ===")
        print(f"忠实度均分: {sum(faith_scores) / len(faith_scores):.2f} / 5")
        print(f"相关性均分: {sum(rel_scores) / len(rel_scores):.2f} / 5")
    return 0
